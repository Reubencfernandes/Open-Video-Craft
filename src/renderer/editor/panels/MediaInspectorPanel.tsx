...
export function MediaInspectorPanel({ item }: { item: EditorMediaItem | null }) {
  ...
  return (
    <div className="grid content-start gap-4">
      <div className="flex items-center gap-3 border-b border-white/[0.07] pb-3"><span className="grid size-9 place-items-center rounded-lg bg-white/[0.08] text-neutral-300"><Icon size={18} /></span><div className="min-w-0"><strong className="block truncate text-sm text-white">{item.name}</strong><span className="text-xs capitalize text-slate-500">{item.kind} media</span></div></div>
